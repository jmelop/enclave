    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="stats">
        ...
        <div className="stat-card">
          ...
          <div className="stat-value mono" style={{ color: totalSaved >= 0 ? 'var(--success)' : 'var(--danger)' }}>
            {fmtSigned(totalSaved)}
          </div>
          <div className="stat-sub">Income minus spending, {months.length} mo</div>
        </div>
        <div className="stat-card">
          <div className="stat-head">
            <div className="stat-icon"><Star size={14} /></div>
      ...
      <Card padding="none">
        ...
        <div style={{ padding: '14px 18px 18px' }}>
          <TrendChart months={months} budgets={budgets} activeIdx={monthIndex} onSelect={setMonthIndex} />
        </div>
      </Card>
            ...
            {rows.slice().reverse().map((r, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setMonthIndex(months.findIndex(x => x.key === r.m.key))}
                >
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13.5, fontWeight: 600 }}>
                    ...
                    {r.m.label} {r.m.year}
                    {r.m.note && (
                      <span className="mono" style={{ fontSize: 9.5, color: 'var(--accent)', border: '1px solid var(--border-subtle)', padding: '1px 5px', borderRadius: 4 }}>
                        {r.m.note}
                      </span>
                    )}
                  </span>
                  <span className="mono" style={{ fontSize: 13, textAlign: 'right', color: 'var(--fg-2)' }}>{fmt(r.income)}</span>
                  <span className="mono" style={{ fontSize: 13, textAlign: 'right', fontWeight: 600, color: r.spent > r.totalBudget ? 'var(--danger)' : 'var(--fg)' }}>
                    {fmt(r.spent)}
                  </span>
                  <span className="mono" style={{ fontSize: 13, textAlign: 'right', fontWeight: 600, color: r.saved >= 0 ? 'var(--success)' : 'var(--danger)' }}>
                    {fmtSigned(r.saved)}
                  </span>
